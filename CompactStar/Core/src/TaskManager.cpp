...
bool Mass_Condition(const MixedStar &in_star)
{
	return ((in_star.GetSequence().d.m + in_star.GetSequence().v.m) >=
		   ...
		   (in_star.GetSequence().d.m + in_star.GetSequence().v.m) <=
			   (J0348_p_0432_M.val + J0348_p_0432_M.err);
}
...
{
	sequence_grid.Import(wrk_dir_ + in_dir);
}
...
void TaskManager::FindCriticalCurve()
{
	...
	// Zaki::Vector::DataSet ds(wrk_dir_ +"/NStar/Dark_Core/Linear_100x100", "Full_Mixed_Seq_Linear_100x100.tsv") ;
	// Zaki::Vector::DataSet ds(wrk_dir_ +"/NStar/Dark_Core/Linear_50x50", "Full_Mixed_Seq_Linear_50x50.tsv") ;
	// Zaki::Vector::DataSet ds(wrk_dir_ +"/NStar/Dark_Core/Linear_256x256", "Full_Mixed_Seq_Linear_256x256.tsv") ;
	// Zaki::Vector::DataSet ds(wrk_dir_ +"/NStar/Dark_Core/testing/No crust/10000_linear_20km", "Seq_10K_L_20km_nocrust_96x96.tsv") ;
	// Zaki::Vector::DataSet ds(wrk_dir_ + "/NStar/Dark_Core/testing/No crust/" + rad_res
	...
	// critical_pt_set.SetWrkDir(wrk_dir_ +
...
void TaskManager::FindMtotContour(const double &in_mass)
{
	...
	critical_curve.Import(wrk_dir_ + "/NStar/Dark_Core/" + chi_str +
						  "/Crit_curve_smooth.tsv");
	...
	else
	{
		...
		Zaki::File::VecSaver vec_saver(wrk_dir_ + "/NStar/Dark_Core/" + chi_str + "/Intersection_" +
									   std::string(tmp) + ".tsv");
								  ...
								  const std::vector<Zaki::Math::Coord2D> &in_m_range)
{
	...
	for (size_t i = 0; i < B_con_set.size(); i++)
	{
		...
		B_curv_stable[B_curv_stable.size() - 1].Export(wrk_dir_ + "/NStar/Dark_Core/" +
													   chi_str + "/B_conts/" +
...
void TaskManager::FindLimits(const double &in_mass) const
{
	...
	for (size_t i = 0; i < cont_divisions; i++)
	{
		...
		darkcore_analysis.FindLimits(in_mass,
									 wrk_dir_ + "/NStar/Dark_Core/" + chi_str + "/B_conts/" +
										 m_str + "/BNV_rates_" + m_str + "_" + std::to_string(i) + ".tsv");
	}

	darkcore_analysis.ExportBNV(wrk_dir_ + "/NStar/Dark_Core/" + chi_str + "/B_conts/" + m_str);
}
